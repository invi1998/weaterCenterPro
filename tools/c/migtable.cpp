...
bool _migratetable()
{
  ...
  // 拼接绑定迁移记录key的字符串。
  char bindstr[2001];  // 绑定迁移记录key的字符串。
  char strtemp[11];
  ...
  while (true)
  {
    ...
    strncpy(keyvalues[ccount],tmpvalue,50);
    ...
    if (ccount == starg.maxcount)
    {
      ...
      // logfile.Write("migrateration %s to %s %d rows in %.2fsec.\n",starg.srctname,starg.dsttname,ccount,Timer.Elapsed());
  ...
  if (stmtsel.m_cda.rpc>0) logfile.Write("migrateration %s to %s %d rows in %.2fsec.\n",starg.srctname,starg.dsttname,stmtsel.m_cda.rpc,Timer.Elapsed());
