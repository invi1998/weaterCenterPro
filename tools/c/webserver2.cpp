...
void *pthmain(void *arg)
{
  ...
  // 判断用户名和密码。
  if (Login(&conn,strrecvbuf,sockfd)==false) { conn.disconnect(); pthread_exit(0); }

  // 判断用户是否有调用接口的权限。
  if (CheckPerm(&conn,strrecvbuf,sockfd)==false) { conn.disconnect(); pthread_exit(0); }
  ...
  if (ExecSQL(&conn,strrecvbuf,sockfd)==false) { conn.disconnect(); pthread_exit(0); }
...
void _help(char *argv[])
{
  printf("Using:/project/tools/bin/webserver2 logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/webserver2 /log/idc/webserver2.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><port>5008</port><timeout>5</timeout>\"\n\n");
