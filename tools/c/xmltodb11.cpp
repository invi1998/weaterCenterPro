/*
 *  程序名：xmltodb11.cpp，本程序是数据中心的公共功能模块，用于把xml文件入库到mysql的表中。
...
void _help(char *argv[])
{
  printf("Using:/project/tools/bin/xmltodb11 logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/xmltodb11 /log/idc/xmltodb11_vip1.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><inifilename>/project/tools/ini/xmltodb.xml</inifilename><xmlpath>/idcdata/xmltodb/vip1</xmlpath><xmlpathbak>/idcdata/xmltodb/vip1bak</xmlpathbak><xmlpatherr>/idcdata/xmltodb/vip1err</xmlpatherr><timetvl>5</timetvl><timeout>50</timeout><pname>xmltodb11_vip1</pname>\"\n\n");
...
int _xmltodb(char *fullfilename,char *filename)
{
  ...
  CFile File;
  if (File.Open(fullfilename,"r")==false) return 3; // 打开xml文件错误。
