...
{
  printf("\n");
  printf("Using:/project/tools/bin/fileclient6 logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools/bin/fileclient6 /tmp/fileclient6.log \"<clienttype>1</clienttype><ip>172.21.0.3</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl>\"\n");
  printf("       /project/tools/bin/fileclient6 /tmp/fileclient6.log \"<clienttype>1</clienttype><ip>49.232.185.49</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl>\"\n\n\n");
...
bool _tcpputfiles()
{
  ...
  while (true)
  {
    ...
    if (SendFile(TcpClient.m_connfd,Dir.m_FullFileName,Dir.m_ModifyTime,Dir.m_FileSize)==true)
    {
      logfile.WriteEx("ok.\n"); delayed++;
    }
    else
    ...
    int buflen=0;
    memset(strRecvBuffer,0,sizeof(strRecvBuffer));
    if (TcpRead(TcpClient.m_connfd,strRecvBuffer,&buflen,-1) == true)
    {
      delayed--;
  ...
  for (int ii=0;ii<delayed;ii++)
  {
    // 接收对端返回的结果。
    memset(strRecvBuffer,0,sizeof(strRecvBuffer));
    if (TcpClient.Read(strRecvBuffer,2000) == true)
    {
      _ackresult(strRecvBuffer); // 接收到对端的文件传输确认报文。
