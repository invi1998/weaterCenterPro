...
int main(int argc,char *argv[])
{
  ...
  // 如果starg.srccols或starg.dstcols为空，就用starg.dsttname表的全部列来填充。
  if ( (strlen(starg.srccols)==0) || (strlen(starg.dstcols)==0) )
  {
    CTABCOLS TABCOLS;

    if (TABCOLS.allcols(&conn,starg.dsttname)==false) 
    {
      logfile.Write("表%s不存在。\n",starg.dsttname); EXIT(-1);
    }

    if (strlen(starg.dstcols)==0) strcpy(starg.dstcols,TABCOLS.m_allcols);
    if (strlen(starg.srccols)==0) strcpy(starg.srccols,TABCOLS.m_allcols);
  }
...
void _help(char *argv[])
{
  printf("Using:/project/tools/bin/syncupdate4 logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/syncupdate4 /log/idc/syncupdate4_ZHOBTCODE2.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><srctname>LK_ZHOBTCODE1</srctname><dsttname>T_ZHOBTCODE2</dsttname><where>where obtid like '59%%%%'</where><synctype>1</synctype><timeout>50</timeout><pname>syncupdate4_ZHOBTCODE2</pname>\"\n\n");

  printf("       /project/tools/bin/procctl 10 /project/tools/bin/syncupdate4 /log/idc/syncupdate4_ZHOBTCODE3.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><srctname>LK_ZHOBTCODE1</srctname><dsttname>T_ZHOBTCODE3</dsttname><srccols>obtid,cityname,provname,lat,height+1,upttime,keyid</srccols><dstcols>obtid,cityname,provname,wd,height,upttime,id</dstcols><where>where obtid like '59%%%%'</where><synctype>2</synctype><srckeycol>keyid</srckeycol><dstkeycol>id</dstkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate4_ZHOBTCODE3</pname>\"\n\n");
  printf("       /project/tools/bin/procctl 10 /project/tools/bin/syncupdate4 /log/idc/syncupdate4_ZHOBTCODE3.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><srctname>LK_ZHOBTCODE1</srctname><dsttname>T_ZHOBTCODE3</dsttname><srccols>obtid,cityname,provname,lat,height+1,upttime,keyid</srccols><dstcols>obtid,cityname,provname,wd,height,upttime,id</dstcols><where>where obtid in ('59849','59855','59948','59954','59981')</where><synctype>2</synctype><srckeycol>keyid</srckeycol><dstkeycol>id</dstkeycol><maxcount>5</maxcount><timeout>50</timeout><pname>syncupdate4_ZHOBTCODE3</pname>\"\n\n");
  ...
  printf("srccols     数据源表的字段列表，用于填充在select和from之间，所以，srccols可以是真实的字段，也可以是函数的返回值或者运算结果。如果本字段为空，就用dsttname表的字段列表填充。\n");
  printf("dstcols     接收数据的目的表的字段列表，与srccols不同，它必须是真实存在的字段。如果本字段为空，就用dsttname表的字段列表填充。\n");
