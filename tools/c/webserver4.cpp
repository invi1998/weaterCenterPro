...
vector<pthread_t> vpthid;  // 存放线程id的容器。
...
int main(int argc,char *argv[])
{
  ...
  {
    pthread_t pthid;
    if (pthread_create(&pthid,NULL,pthmain,(void *)(long)ii)!=0)
...
void mainexit(int sig)
{
  ...
  // 取消全部的线程。
  for (int ii=0;ii<vpthid.size();ii++) pthread_cancel(vpthid[ii]);
...
void _help(char *argv[])
{
  printf("Using:/project/tools/bin/webserver4 logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/webserver4 /log/idc/webserver4.log \"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>gbk</charset><port>5008</port><timeout>5</timeout>\"\n\n");
