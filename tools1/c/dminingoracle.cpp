...
7、如果SQL语句有问题，mysql的prepare()会返回错误，oracle的prepare()不会返回错误，为了兼容性考虑，尽可能不判断prepare()的返回值；
...
int imaxfieldlen=-1;   // 结果集字段值的最大长度，存放fieldlen数组中元素的最大值。

char strfieldname[MAXPARAMS][31];    // 结果集字段名数组，从starg.fieldstr解析得到。
...
bool _dminingoracle()
{
  ...
  char strfieldvalue[ifieldcount][imaxfieldlen+1];  // 抽取数据的SQL执行后，存放结果集字段值的数组。
...
void _help()
{
  printf("Using:/project/tools1/bin/dminingoracle logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE1.log \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE1</selectsql><fieldstr>obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><timeout>30</timeout><pname>dminingoracle_ZHOBTCODE1</pname>\"\n\n");
  printf("       /project/tools1/bin/procctl   30 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND1.log \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND1 where keyid>:1 and ddatetime>sysdate-0.04</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><starttime></starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/dminingoracle_ZHOBTMIND1_HYCZ.list</incfilename><timeout>30</timeout><pname>dminingoracle_ZHOBTMIND1_HYCZ</pname><maxcount>1000</maxcount><connstr1>qxidc/qxidcpwd@snorcl11g_132</connstr1>\"\n\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  for (int ii=0;ii<CmdStr.CmdCount();ii++)
  {
    ...
    // if (ifieldlen[ii]>imaxfieldlen) ifieldlen[ii]=imaxfieldlen;   // 字段的长度不能超过imaxfieldlen。
    if (ifieldlen[ii]>imaxfieldlen) imaxfieldlen=ifieldlen[ii];   // 得到字段长度的最大值。
  }
