...
void crtsql()
{
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    // 拼接strinsertp2，需要区分keyid字段和date字段。
  ...
  // 生成修改表的SQL语句。
  // update T_ZHOBTMIND1 set t=:1,p=:2,u=:3,wd=:4,wf=:5,r=:6,vis=:7,upttime=now(),mint=:8,minttime=to_date(:9,'yyyymmddhh24miss') where obtid=:10 and ddatetime=to_date(:11,'yyyymmddhh24miss')
